# nossa classe Lista atende as principais operações que essa TAD nos oferece, mas que tal melhorarmos?
...
"""
1 - a. clear(self)
    - Existem 2 possibilidades de respostas a serem implementadas.
    #1 só funciona, sem que ocorra vazamento de memória, graças ao garbage collector do python, porém, o mais indicado é a opção #2:
    #2 pois utilizam a própria estrutura para atribuir um novo comportamento.

1 - b. __get_node_at(self, position)
    - Para este desafio foi realizado a extração do trecho de código mais repetitivo, que estava sendo utilizado nos demais métodos.
    Vale salientar que nos casos que precisamos do anterior ao ultimo, precisamos fazer a operação:
    len(self) - 2
"""
...
""" Resultado """
# └─# python3 linked_list_content_1.py

# True
# LinkedList(len=1, value=Node(value=1, next=None))
# LinkedList(len=2, value=Node(value=2, next=Node(value=1, next=None)))
# LinkedList(len=3, value=Node(value=2, next=Node(value=1, next=Node(value=3, next=None))))
# LinkedList(len=2, value=Node(value=2, next=Node(value=1, next=None)))
# LinkedList(len=1, value=Node(value=1, next=None))
