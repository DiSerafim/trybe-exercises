from node_double import DoublyNode
...
class DoublyLinkedList:
    ...
    def __get_node_at(self, position):
        if position < self.__length / 2:
            return self.__get_node_at_forward(position)
        else:
            return self.__get_node_at_backwards(position)

    def __get_node_at_forward(self, position):
    ...
    def __get_node_at_backwards(self, position):
        backwards_position = self.__length - position
        value_to_be_returned = None

        if self.head.next != self.tail:
            value_to_be_returned = self.tail.previous
            while backwards_position > 0:
                value_to_be_returned = value_to_be_returned.previous
                backwards_position -= 1
        return value_to_be_returned
