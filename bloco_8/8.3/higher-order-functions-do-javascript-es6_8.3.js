...
function fantasyOrScienceFiction() {
  // escreva seu código aqui
  return books.filter((select) => {
    return select.genre === 'Ficção Científica' || select.genre === 'Fantasia';
  });
}
...
function fantasyOrScienceFictionAuthors() {
  // escreva seu código aqui
  const findAllMembers = books.filter((fictionFantasy) =>
  fictionFantasy.genre === 'Ficção Científica' || fictionFantasy.genre === 'Fantasia'
  );
...
function authorWith3DotsOnName() {
  // escreva seu código aqui
  const searchForDot = books.find((book) =>
    book.author.name[1] === '.' && book.author.name[4] === '.' && book.author.name[7] === '.'
  );
  // console.log(searchForDot['name']);
  return searchForDot['name']
}
